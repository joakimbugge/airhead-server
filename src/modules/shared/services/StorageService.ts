...
export class StorageService {
  ...
  constructor(private readonly config: ConfigService) {
    const { DO_SPACES_REGION, DO_SPACES_URL, DO_SPACES_ACCESS_KEY, DO_SPACES_SECRET_KEY } = this.config.env;
  ...
  public composeCdnUrl(path: string): string {
    const { DO_SPACES_NAME, DO_SPACES_REGION, DO_SPACES_URL } = this.config.env;
